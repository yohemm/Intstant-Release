...
echo "d" > file.txt
git commit -q -am "refactor: cleanup"

echo "e" > file.txt
git commit -q -am "BREAKING CHANGE: api"
...
assert_contains "$CHANGELOG_CONTENT" "### Breaking Changes"
assert_contains "$CHANGELOG_CONTENT" "### Features"
assert_contains "$CHANGELOG_CONTENT" "### Fixes"
assert_contains "$CHANGELOG_CONTENT" "### Refactors"

echo "ok - changelog sections"
